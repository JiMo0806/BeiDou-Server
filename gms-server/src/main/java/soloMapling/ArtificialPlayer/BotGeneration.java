...
public class BotGeneration {
    ...
    // Bounding concurrent createBot calls: loadCharFromDB holds a Druid
    // connection while calling MapManager.getMap() for the template's saved
    // map, and MapManager.loadMapFromWz is synchronized + needs a connection
    // itself. Unbounded parallel waves (virtual threads) can therefore exhaust
    // the Druid pool with connection-holding threads all blocked on the map
    // lock, deadlocking the server. Keeping bot-spawn DB work under the pool
    // size always leaves connections free for map loads (fix: see also
    // warmTemplateCharacter and druid max-active in application.yml).
    private static final Semaphore CREATE_BOT_PERMITS = new Semaphore(6);
    ...
    /**
     * Template character that all bots are cloned from. Original SoloMapling
     * hardcoded CID 2, which collides with real characters on production
     * databases - V900.0.2 provisions a dedicated template at a safe high ID.
     * Override via game_config: server/bot_template_cid.
     */
    public static final int DEFAULT_BOT_TEMPLATE_CID = 990000001;

    public static int getBotTemplateCid() {
        return GameConfig.get("server", "bot_template_cid", DEFAULT_BOT_TEMPLATE_CID);
    }
    ...
    public static Character getConsoleBot() {
        ...
        int botId = 999;
        int baseId = getBotTemplateCid(); // Base Bot Character (see V900.0.2)
    ...
    public static int createBot(Point pos, MapleMap map, int baseClass, int minLevel, int maxLevel, int forcedJobId) {
        try {
            CREATE_BOT_PERMITS.acquire();
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("createBot interrupted while waiting for a spawn permit", e);
        }
        try {
            return doCreateBot(pos, map, baseClass, minLevel, maxLevel, forcedJobId);
        } finally {
            CREATE_BOT_PERMITS.release();
        }
    }

    private static int doCreateBot(Point pos, MapleMap map, int baseClass, int minLevel, int maxLevel, int forcedJobId) {
        int cid = getBotTemplateCid(); // Base Bot Character (see V900.0.2)
    ...
    /**
     * Loads the template character once, serially, before any parallel wave
     * runs. loadCharFromDB calls MapManager.getMap() for the character's saved
     * map while holding a DB connection; if that map is not yet cached, the
     * synchronized loadMapFromWz needs another connection while every parallel
     * bot spawn holds one - classic pool-exhaustion deadlock. Warming the
     * template's map here means every later loadCharFromDB hits the map cache
     * (plain read-lock, no DB) and the deadlock window disappears.
     */
    public static void warmTemplateCharacter() {
        try {
            long start = System.currentTimeMillis();
            Character.loadCharFromDB(getBotTemplateCid(), getBotClient(), false);
            System.out.println(String.format(
                    "[BotGeneration] Template character %d warmed in %dms (template map now cached)",
                    getBotTemplateCid(), System.currentTimeMillis() - start));
        } catch (Exception e) {
            System.err.println("[BotGeneration] Template character warm-up failed: " + e);
            e.printStackTrace();
        }
    }
